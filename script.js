...
const informationButtonEl = document.querySelector(".information-button");
const informationContainerEl = document.querySelector(".information-modal");
const closeInformationButtonEl = document.querySelector(
  ".close-information-button"
);
const musicImageEl = document.querySelector(".music-image");
const soundImages = ["./menu/mute.png", "./menu/music.png"];
informationButtonEl.addEventListener("click", () => {
  informationContainerEl.classList.toggle("information-modal-active");
...
closeInformationButtonEl.addEventListener("click", () => {
  informationContainerEl.classList.toggle("information-modal-active");
...
let muteCounter = 0;
const gameMenuAudio = new Audio();
gameMenuAudio.src = "./sound/menu.mp3";
gameMenuAudio.muted = false;
gameMenuAudio.play();

musicButtonEl.addEventListener("click", () => {
  if (muteCounter % 2 === 0) {
    musicImageEl.src = soundImages[0];
    muteCounter++;
    gameMenuAudio.pause();
    console.log(muteCounter);
  } else {
    musicImageEl.src = soundImages[1];
    muteCounter++;
    gameMenuAudio.play();
...
restartGameButtonEl.addEventListener("click", () => {
  ...
  userCorrectAnswerEl.textContent = 0;
  userWrongAnswerEl.textContent = 0;
